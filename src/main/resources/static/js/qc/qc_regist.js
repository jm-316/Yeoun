...
document.addEventListener("DOMContentLoaded", () => {
	...
	function startQcAndOpenModal(rowData) {
	  ...
	  fetch(`${contextPath}/qc/start?orderId=${encodeURIComponent(orderId)}`, {
	    method: "POST",
	    headers: {
...
function loadQcRegistGrid() {
    fetch(`${contextPath}/qc/regist/data`)
        .then(res => res.json())
        .then(data => {
...
function loadQcDetailRows(qcResultId) {
  ...
  fetch(`${contextPath}/qc/${qcResultId}/details`)
    .then((res) => {
      if (!res.ok) {
...
function onClickSaveQcResult() {
	...
	// 5) fetch 호출
	fetch(`${contextPath}/qc/${qcResultId}/save`, {
	  method: "POST",
	  headers: {
...
//      })
  ...
  function cancelQc(orderId) {
	...
	fetch(`${contextPath}/qc/cancel?orderId=${encodeURIComponent(orderId)}`, {
	  method: "POST",
	  headers: {
	    ...(csrfToken && csrfHeaderName ? { [csrfHeaderName]: csrfToken } : {})
	  }
  ...
  function uploadQcDetailFilesPromise(qcResultDtlId, files) {
    ...
    return fetch(`${contextPath}/qc/detail/${qcResultDtlId}/files`, {
      method: "POST",
      headers: {
        ...(csrfToken && csrfHeaderName ? { [csrfHeaderName]: csrfToken } : {})
      },
