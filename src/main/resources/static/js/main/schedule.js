...
function patchApproval(btn) {
	...
	if(confirm(msg)) {
		//결재 확인 동작함수
		fetch(`${contextPath}/api/approvals/${approvalId}?btn=${btn}` , {
			method: 'PATCH'
			, headers: {
...
async function getApproverList(approvalId) {
	try {
		const response = await fetch(`${contextPath}/api/approvals/approvers/${approvalId}`, {method: 'GET'});
...
async function getScheduleData(params) {
	await fetch(`${contextPath}/api/schedules?${params.toString()}`, {method: 'GET'})
	.then(response => {
		if (!response.ok) throw new Error(response.text());
		return response.json();  //JSON 파싱
	})
...
async function getLeaveData(params) {
	await fetch(`${contextPath}/api/schedules/leaves?${params.toString()}`, {method: 'GET'})
	.then(response => {
		if (!response.ok) throw new Error(response.text());
		return response.json();  //JSON 파싱
	})
...
async function getLastNoticeList() {
	await fetch(`${contextPath}/api/notices/last-notice`, {method: 'GET'})
	.then(response => {
		if (!response.ok) throw new Error(response.text());
		return response.json();  //JSON 파싱
	}).then(data => {
...
async function getApprovalList() {
	await fetch(`${contextPath}/api/approvals`, {method: 'GET'})
	.then(response => {
		if (!response.ok) throw new Error(response.text());
		return response.json();  //JSON 파싱
	}).then(data => {
...
//	console.log(data, "공지데이터");
	...
	const response = await fetch(`${contextPath}/approval/empList`);
