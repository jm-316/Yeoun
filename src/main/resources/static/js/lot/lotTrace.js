...
async function loadProductRootDetail(lotNo) {
  ...
  try {
    const html = await fetchText(`${contextPath}/lot/trace/detail?lotNo=${encodeURIComponent(lotNo)}`);
...
function loadMaterialRootDetail(lotNo) {
  ...
  fetch(`${contextPath}/lot/trace/material-root-detail?lotNo=${encodeURIComponent(lotNo)}`)
    .then(res => {
...
async function loadProcessList(container, lotNo) {
  ...
  try {
    const list = await fetchJson(`${contextPath}/lot/trace/process-list?lotNo=${encodeURIComponent(lotNo)}`);
    container.innerHTML = "";
...
async function loadMaterialList(container, lotNo) {
  ...
  try {
    const list = await fetchJson(`${contextPath}/lot/trace/material-list?lotNo=${encodeURIComponent(lotNo)}`);
    container.innerHTML = "";
...
async function loadUsedProductList(container, inputLotNo) {
  ...
  try {
    const list = await fetchJson(`${contextPath}/lot/trace/material/used-products?inputLotNo=${encodeURIComponent(inputLotNo)}`);
    container.innerHTML = "";
...
async function loadProcessDetail(orderId, stepSeq) {
  try {
    const detail = await fetchJson(
      `${contextPath}/lot/trace/process-detail?orderId=${encodeURIComponent(orderId)}&stepSeq=${encodeURIComponent(stepSeq)}`
    );
...
async function loadMaterialDetail(outputLotNo, inputLotNo) {
  try {
    const detail = await fetchJson(
      `${contextPath}/lot/trace/material-detail?outputLotNo=${encodeURIComponent(outputLotNo)}&inputLotNo=${encodeURIComponent(inputLotNo)}`
    );
