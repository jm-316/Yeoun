...
function loadProcessGrid() {
  ...
  fetch(`${contextPath}/process/status/data?${params.toString()}`)
    .then(async (res) => {
      if (!res.ok) {
...
function openDetailModal(orderId) {
  ...
  fetch(`${contextPath}/process/status/detail/${orderId}`)
    .then((res) => {
...
function handleStartStep(orderId, stepSeq) {
  ...
  fetch(`${contextPath}/process/status/step/start`, {
    method: 'POST',
    headers,
...
function finishStepWithQty(orderId, stepSeq, goodQty, defectQty) {
  ...
  fetch(`${contextPath}/process/status/step/finish`, {
    method: 'POST',
    headers: headers,
    body: JSON.stringify({
      orderId,
...
function handleSaveStepMemo(orderId, stepSeq, memo, inputEl) {
  ...
  fetch(`${contextPath}/process/status/step/memo`, {
    method: 'POST',
    headers: headers,
